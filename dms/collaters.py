...
class OAMaskCollater(object):
    ...
    def __call__(self, sequences):
        ...
        tokenized = _pad(tokenized, self.tokenizer.pad_id)
        print(src.shape, masks.shape)
        return (src.to(torch.long), timesteps, tokenized.to(torch.long), masks)

class DMCollater(object):
    """
    Collater for masking sequences:
    - D3PM according to Austin et al.
    - OAARDMs according to Hoogeboom et al.
    inputs:
        sequences : list of sequences
        inputs_padded: if inputs are padded (due to truncation in Simple_Collater) set True (default False)
        scheme: 'OA' or 'D3PM' Order Agnostic or D3PM masking scheme
    ...
    outputs:
        ...
        masks: masks used to generate src
    """
    def __init__(self, simple_collater, inputs_padded=False, num_timesteps=1000, masking_scheme='random', schedule='cosine'):
        self.simple_collater = simple_collater
        self.tokenizer = Tokenizer()
        self.inputs_padded  = inputs_padded
    ...
    def __call__(self, sequences):
        ...
        # Make time schedule
        if self.masking_scheme == 'random':
            betas = _beta_schedule(self.num_timesteps, schedule=self.schedule)
            t_betas = [betas[step] for step in range(self.num_timesteps)]
        elif self.masking_scheme == 'blosum62':
            d = Blosum62().blosum_dict()
            betas = _beta_schedule(self.num_timesteps, schedule=self.schedule, start=min(d.values()), end=max(d.values()))
            t_betas = [betas[step] for step in range(self.num_timesteps)]

        for i, x in enumerate(tokenized):
            #print("seq", i)
            if self.inputs_padded:
                 x = _unpad(x, self.tokenizer.pad_id)
            # x_t = x.clone()
            if self.masking_scheme == 'random':
                 x_prob = torch.rand_like(x.float())
            elif self.masking_scheme == 'blosum62':
                 x_prob = blosum_probability(x.float())
            else:
                 print("Select random or blosum62 for masking scheme")
        ...
        masks = _pad(masks*1, 0)
        tokenized = _pad(tokenized, self.tokenizer.pad_id)
        return (src.to(torch.long), tokenized.to(torch.long), masks)
